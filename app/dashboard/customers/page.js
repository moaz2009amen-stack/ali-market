...
export default function CustomersPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isOnline()) {
      ...
      if (editMode && currentCustomer) {
        ...
        await addToQueue('customers', 'update', { id: currentCustomer.id, ...customerData })
        toast.success('✓ تم التعديل — سيُرفع عند عودة الإنترنت')
      } else {
        const newCustomer = { id: localUUID(), ...customerData, total_debt: 0, created_at: new Date().toISOString() }
        const updated = [...customers, newCustomer]
        setCustomers(updated)
        await saveToCache('customers', updated)
        await addToQueue('customers', 'insert', newCustomer)
        toast.success('✓ تم الإضافة — سيُرفع عند عودة الإنترنت')
      }
      closeModal()
      return
    }
    ...
    try {
      ...
      if (editMode && currentCustomer) {
        ...
        const { data, error } = await supabase.from('customers')
          .insert([{ ...customerData, created_by: user.id }]).select().single()
        if (error) throw error
  ...
  const columns = [
    ...
    { header: 'الهاتف',   accessor: 'phone',  render: r => <span className="font-mono text-sm">{formatPhone(r.phone)}</span> },
    ...
    { header: 'إجراءات',  accessor: 'actions', render: r => (
      <div className="flex gap-1">
        ...
        {!offline && <button onClick={() => handleDelete(r)} className="p-2 text-danger-600 hover:bg-danger-50 rounded-lg"><Trash2 size={16} /></button>}
      </div>
    )},
  ]
    ...
    <div className="space-y-4 sm:space-y-6">
      {offline && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-center gap-2">
          ...
          <span className="text-yellow-800 text-sm font-medium">بدون إنترنت — الإضافة والتعديل يُحفظان محلياً</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        <Card padding={false} className="p-4 sm:p-6">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-gray-600">إجمالي العملاء</p><p className="text-2xl font-bold text-gray-900">{stats.total}</p></div>
      ...
      <Card title="العملاء" action={
        ...
        <div className="md:hidden space-y-3">
          {customers.length === 0 ? <div className="text-center py-8 text-gray-500">لا يوجد عملاء</div>
          : customers.map(c => (
            <div key={c.id} className="bg-gray-50 rounded-lg p-4">
              <div className="flex items-start justify-between mb-2">
                <div><p className="font-semibold text-gray-900">{c.name}</p>{c.area && <p className="text-xs text-gray-500">{c.area}</p>}</div>
              ...
              <div className="flex gap-2">
                ...
                {!offline && <button onClick={() => handleDelete(c)} className="flex-1 flex items-center justify-center gap-1 px-3 py-2 text-danger-600 bg-danger-50 rounded-lg text-sm"><Trash2 size={15} /> حذف</button>}
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title={editMode ? 'تعديل عميل' : 'إضافة عميل جديد'} size="md">
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="اسم العميل" name="name" value={formData.name} onChange={e => setFormData(p => ({...p, name: e.target.value}))} required error={errors.name} />
          <Input label="رقم الهاتف" type="tel" name="phone" value={formData.phone} onChange={e => setFormData(p => ({...p, phone: e.target.value}))} placeholder="01001234567" required error={errors.phone} />
          <Input label="المنطقة (اختياري)" name="area" value={formData.area} onChange={e => setFormData(p => ({...p, area: e.target.value}))} />
          <Input label="العنوان (اختياري)" name="address" value={formData.address} onChange={e => setFormData(p => ({...p, address: e.target.value}))} />
          <div className="flex gap-3 pt-2">
            <Button type="submit" variant="primary" fullWidth>{editMode ? 'تحديث' : offline ? '✓ إضافة محلياً' : 'إضافة العميل'}</Button>
